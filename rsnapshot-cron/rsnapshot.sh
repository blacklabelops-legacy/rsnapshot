...
[ "$hourly_times" -gt 0 ] && echo -e retain'\t'hourly'\t'$hourly_times >> /etc/rsnapshot.conf
...
[ "$daily_times" -gt 0 ] && echo -e retain'\t'daily'\t'$daily_times >> /etc/rsnapshot.conf
...
[ "$weekly_times" -gt 0 ] && echo -e retain'\t'weekly'\t'$weekly_times >> /etc/rsnapshot.conf
...
[ "$monthly_times" -gt 0 ] && echo -e retain'\t'monthly'\t'$monthly_times >> /etc/rsnapshot.conf
